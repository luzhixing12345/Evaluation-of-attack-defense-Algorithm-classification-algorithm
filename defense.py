...
def start_defense(model,cfg,loss_fn,test_set):
    ...
    for data,target in test_set:
        ...
        def_data,def_config = defense(new_data)         #返回tensorflow的tensor
    ...
    def_result = test_def(model._inner,def_test_data,loss_fn,cfg,tf= True)
    cfg.defense_arguments= def_config
    return def_result
...
def DEF_BDR(new_data):
    ...
    return bit_depth_reduce(new_data, x_min=0.0, x_max=1.0,step_num=5,alpha=1e6),config

def DEF_JPEG(new_data):
    config = {
        'quality':95
    }
    return jpeg_compress(new_data, x_min=0.0, x_max=1.0,quality=95),config
